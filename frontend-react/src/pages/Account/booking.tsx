...
export default function BookingPage() {
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`px-2.5 py-1 rounded-sm text-xs font-semibold ${STATUS_BADGE[item.status] ?? 'bg-gray-100 text-gray-600'}`}>
                          {STATUS_LABEL[item.status] ?? item.status}
                        </span>
                      </div>
                      <p className="text-sm text-gray-500 mt-1">{item.date} · {item.time} · {item.guestCount} khách</p>
                    </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mt-1">Thông tin đặt bàn</h1>
          </div>
          <span className={`inline-flex w-fit items-center px-4 py-2 rounded-xl text-sm font-bold shadow-sm ring-1 ring-inset ${STATUS_BADGE[booking.status] ?? 'bg-gray-100 text-gray-600'} ring-current/15`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 bg-white border border-red-100 rounded-xl shadow-sm p-6 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 rounded-xl bg-linear-to-br from-red-50 via-white to-slate-50 border border-red-100 p-5">
              ...
              {(booking.status === 'confirmed' || booking.status === 'completed') && <button onClick={() => setIsReportOpen(true)} className="mt-2 rounded-xl border border-red-200 bg-white px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50">Báo cáo nhà hàng</button>}
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <InfoCard label="Tổng số khách" value={`${booking.guestCount} người`} />
              <InfoCard label="Trẻ em đi cùng" value={`${booking.childCount} trẻ em`} />
              <InfoCard label="Số chỗ ngồi" value={`${booking.requestSeats} chỗ`} />
            </div>
              ...
              {booking.booking_items.length === 0 ? (
                ...
                <div className="space-y-3">
                  {booking.booking_items.map((item) => (
                    <div key={item.bookingItemId} className="flex items-center justify-between gap-4 rounded-xl border border-gray-100 p-4">
                      <div className="min-w-0">
                        <p className="font-semibold text-gray-900 truncate">{item.name}</p>
                        <p className="text-xs text-gray-400 mt-1">{getCategoryLabel(item.category)}</p>
                      </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-5 space-y-3">
              ...
              <DetailRow label="Ngày tạo" value={formatCreatedAt(booking.createdAt)} />
      ...
      {isReportOpen && <ViolationReportModal bookingId={booking.bookingId} target="restaurant" onClose={() => setIsReportOpen(false)} onSuccess={() => void bookingDetailQ.refetch()} />}
    </div>
  );
}
