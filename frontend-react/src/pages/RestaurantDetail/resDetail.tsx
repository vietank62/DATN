...
export const RestaurantDetail = () => {
  ...
  const [createdBooking, setCreatedBooking] = useState<BookingDetail | null>(
    null,
  );
  ...
  const {
    ...
    error: overviewError,
  } = useQuery<RestaurantOverview>({
    queryKey: ["restaurant-overview", id],
    queryFn: () =>
      api
        .get<RestaurantOverview>(`/v1/restaurants/${id}/overview`)
  ...
  const { data: restaurantMenuItems } = useQuery<RestaurantMenuItem[]>({
    ...
    queryFn: async () => {
      const { data } = await api.get<RestaurantMenuItem[]>(
        `/v1/menuitems/restaurant/${id}`,
      );
      return data;
    },
    enabled: !!id && (activeTab === "Thực đơn" || isModalOpen),
    staleTime: 1000 * 60 * 5,
  });

  const { data: restaurantReviews = [], isLoading: isReviewsLoading } =
    useQuery<RestaurantReview[]>({
      ...
      enabled: !!id && Boolean(restaurantOverview),
      staleTime: 1000 * 60,
    });

  const favoriteStatusQueryKey = [
    "restaurant-favorite",
    id,
  ...
  const { data: favoriteStatus } = useQuery<{ is_favorite: boolean }>({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ is_favorite: boolean }>(
        `/v1/favorites/check/${id}`,
      );
      return data;
    },
    enabled: !!id && isAuthenticated && Boolean(restaurantOverview),
    staleTime: 1000 * 60 * 5,
  });
  ...
  const toggleFavoriteMutation = useMutation({
    mutationFn: async (restaurantId: number) => {
      const { data } = await api.post<{
        ...
        like_count: number;
      }>("/v1/favorites/toggle", { restaurantId });
      return data;
    },
    onMutate: async (restaurantId: number) => {
      ...
      const previousRestaurantBase = queryClient.getQueryData<RestaurantCard>([
        "restaurant-base",
        id,
      ]);
      ...
      const previousRestaurantLists = queryClient.getQueriesData<
        RestaurantCard[]
      >({
      ...
      queryClient.setQueriesData<RestaurantCard[]>(
        { queryKey: ["restaurants"] },
        (oldData) => {
          if (!oldData) return oldData;
      ...
      return {
        previousRestaurantBase,
        previousFavoriteStatus,
        previousRestaurantLists,
      };
    },
    ...
    onError: (error: unknown, _restaurantId, context) => {
      if (context?.previousRestaurantBase) {
        queryClient.setQueryData(
          ["restaurant-base", id],
          context.previousRestaurantBase,
        );
      }
    ...
    onError: (error: unknown) => {
      ...
      toast.error(
        axiosError.response?.data?.detail ||
          "Không thể tạo đơn đặt bàn, vui lòng thử lại.",
      );
    },
  });
      ...
      <div className="w-full min-h-screen bg-slate-100 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold text-gray-500 animate-pulse">
            Đang tải thông tin chi tiết nhà hàng...
          </p>
        </div>
      </div>
    );
  ...
  if (overviewError || !restaurantBase || !restaurantDetail) {
    return (
      <div className="w-full min-h-screen bg-slate-100 flex flex-col items-center justify-center gap-4">
        ...
        <button
          onClick={() => navigate("/")}
          className="px-5 py-2.5 bg-red-600 text-white rounded-xl text-xs font-bold transition-transform hover:scale-105"
        >
          Quay lại trang chủ
        </button>
      </div>
  ...
  const albumImages =
    restaurantDetail.image_urls && restaurantDetail.image_urls.length > 0
  ...
  const customerContentClassName =
    "whitespace-pre-wrap text-sm font-medium leading-7 text-gray-700";
  ...
  const groupedMenuItems = (restaurantMenuItems ?? []).reduce<
    Record<string, RestaurantMenuItem[]>
  >((groups, item) => {
  ...
  const selectedMenuTotal = selectedMenuItemEntries.reduce((total, entry) => {
    const menuItem = restaurantMenuItems?.find(
          ...
          <div className="mx-auto grid max-w-7xl grid-cols-1 gap-3 lg:grid-cols-12 lg:gap-5">
            <div
              ...
              className={`relative h-80 overflow-hidden rounded-2xl border border-gray-200/50 bg-gray-900 shadow-sm select-none md:h-97.5 lg:h-110 ${
                hasSideImages ? "lg:col-span-8" : "lg:col-span-8 lg:col-start-3"
              }`}
            >
              <img
                    ...
                    {idx === sideImages.length - 1 &&
                      albumImages.length > 5 && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/50 p-2 text-white backdrop-blur-[1px]">
                          ...
                          <span className="mt-0.5 text-xs text-gray-200">
                            +{albumImages.length - 5} ảnh
                          </span>
                        </div>
                      )}
                  </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 w-full items-start">
          <div className="lg:col-span-2 bg-white rounded-2xl p-6 border border-gray-100 shadow-xs flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  <button
                    ...
                    onClick={() =>
                      toggleFavoriteMutation.mutate(restaurantBase.id)
                    }
                    aria-label={
                  ...
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill={
                        favoriteStatus?.is_favorite ? "currentColor" : "none"
                      }
                      viewBox="0 0 24 24"
                      strokeWidth={2}
                      stroke="currentColor"
            ...
            {(categoryLabels.length > 0 ||
              (restaurantBase.suitable_for?.length ?? 0) > 0 ||
              (restaurantBase.service_types?.length ?? 0) > 0) && (
            ...
            <div className="flex border-b border-gray-200 overflow-x-auto -mx-6 px-6 justify-between">
              {["Mô tả", "Thực đơn", "Tiện ích", "Bãi xe", "Quy định"].map(
                (tab) => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`py-3 px-4 text-xs md:text-sm font-bold border-b-2 transition-all whitespace-nowrap ${activeTab === tab ? "border-red-600 text-red-600" : "border-transparent text-gray-500"}`}
                  >
                    {tab}
                  </button>
                ),
            ...
            <div className="py-2 text-sm text-gray-700 leading-relaxed min-h-40">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {items.map((item) => (
                          <div
                            key={item.id}
                            className="overflow-hidden rounded-xl border border-gray-200 bg-white p-3 flex gap-3"
                          >
                            <img
              ...
              {activeTab === "Bãi xe" && (
                <p className={customerContentClassName}>
                  {restaurantDetail.parking_info ||
                    "Nhà hàng chưa cập nhật thông tin bãi đỗ xe."}
                </p>
              )}
              {activeTab === "Quy định" && (
                <p className={customerContentClassName}>
                  {restaurantDetail.regulations ||
                    "Nhà hàng chưa cập nhật quy định."}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden sticky top-30">
            <div className="bg-white p-5 border-b border-gray-100 text-center flex flex-col gap-1">
              ...
              <p className="text-xs text-gray-400">
                Chọn thời gian và số lượng khách để giữ chỗ nhé
              </p>
            </div>
            <form
              onSubmit={handleOpenNextStep}
            ...
            >
                ...
                <div className="grid grid-cols-5 gap-1.5">
                  {quickBookingTimes.map((time) => (
                    <button
                      ...
                      className={`rounded-lg border py-2 text-[11px] font-semibold transition-colors ${bookingTime === time ? "border-red-600 bg-red-50 text-red-600" : "border-gray-200 bg-white text-gray-600 hover:border-red-200"}`}
                    >
                      {time}
                    </button>
                  ))}
              ...
              <button
                type="submit"
                className="w-full bg-red-600 hover:bg-red-700 text-white text-sm font-bold py-3.5 rounded-lg shadow-md transition-colors cursor-pointer mt-2"
              >
                Đặt bàn ngay
              </button>
              <button
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full max-h-[calc(100vh-2rem)] overflow-hidden shadow-2xl border border-gray-100 flex flex-col">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Hoàn thiện thông tin để gửi yêu cầu đến nhà hàng
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="p-5 sm:p-6 overflow-y-auto min-h-0 flex flex-col gap-5">
              <div
                ...
                aria-label={`Bước ${bookingStep} trên 4`}
              >
                {[1, 2, 3, 4].map((step) => (
                  <span
                    key={step}
                    className={`h-1.5 flex-1 rounded-full ${step <= bookingStep ? "bg-red-600" : "bg-gray-200"}`}
                  />
                ))}
              </div>
              {bookingStep === 1 && (
                <div className="flex flex-col gap-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Bạn có thể điều chỉnh lại số khách, ngày và giờ đến trước
                      khi tiếp tục.
                    </p>
                  </div>
                    ...
                    <div className="grid grid-cols-5 gap-1.5">
                      {quickBookingTimes.map((time) => (
                        <button
                          ...
                          type="button"
                          onClick={() => setBookingTime(time)}
                          className={`rounded-lg border py-2 text-[11px] font-semibold ${bookingTime === time ? "border-red-600 bg-white text-red-600" : "border-transparent bg-white text-gray-600"}`}
                        >
                          {time}
                        </button>
                      ))}
                  ...
                  <button
                    ...
                    className="w-full bg-red-600 hover:bg-red-700 text-white text-sm font-bold py-3 rounded-xl shadow-md transition-colors"
                  >
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button
                      ...
                      className="w-1/3 bg-gray-100 text-gray-700 text-xs font-bold py-3 rounded-xl"
                    >
                      Quay lại
                    </button>
                    <button
                      ...
                      className="w-2/3 bg-red-600 text-white text-xs font-bold py-3 rounded-xl"
                    >
                      Tiếp tục: chỗ ngồi & món ăn
                    </button>
                  </div>
                </div>
              )}
              ...
              {bookingStep === 3 && (
                <>
                  <div className="flex flex-col gap-3">
                    <div>
                      <h4 className="text-sm font-bold text-gray-900">
                        Chọn chỗ ngồi và món ăn
                      </h4>
                      <p className="text-xs text-gray-400 mt-1">
                        Các lựa chọn này giúp nhà hàng chuẩn bị chu đáo hơn.
                      </p>
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-xs text-gray-500 font-semibold">
                        Số lượng chỗ ngồi mong muốn
                      </label>
                      <input
                        type="number"
                        min={adults + children}
                      ...
                      {(restaurantMenuItems ?? []).length === 0 ? (
                        ...
                        (restaurantMenuItems ?? []).map((item) => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className="flex items-center justify-between gap-3 rounded-lg bg-white border border-gray-100 p-2.5 shadow-2xs"
                            >
                                ...
                                {quantity > 0 ? (
                                  <>
                                    <button
                                      ...
                                      className="w-6 h-6 rounded border border-gray-200 bg-white font-bold text-xs"
                                    >
                                      -
                                    </button>
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-gray-500 font-semibold">
                      Ghi chú đặc biệt gửi tới nhà hàng
                    </label>
                    <textarea
                      value={note}
                  ...
                  <div className="flex gap-3 pt-2 border-t border-gray-100">
                    ...
                    <button
                      ...
                      className="w-2/3 bg-red-600 disabled:opacity-60 text-white text-xs font-bold py-3 rounded-xl"
                    >
              ...
              {bookingStep === 4 && (
                <div className="flex flex-col gap-4 text-center py-2">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      navigate(`/account/bookings/${createdBooking?.bookingId}`)
                    }
      ...
      {isLightboxOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 group select-none">
          ...
          <button
            ...
            className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center shadow-lg transition-all cursor-pointer"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
          ...
          <button
            ...
            className="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center shadow-lg transition-all cursor-pointer"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
          ...
          <div className="absolute bottom-6 flex items-center gap-1.5 bg-black/60 text-white text-xs px-3 py-1.5 rounded-full font-medium">
            <Camera className="h-3.5 w-3.5" /> {lightboxIndex + 1} /{" "}
            {albumImages.length}
          </div>
        </div>
      )}
