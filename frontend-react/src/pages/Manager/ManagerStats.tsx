...
// ── Types ─────────────────────────────────────────────────────────────────────
interface MonthlyBookingStat { month: number; count: number; label: string; }
interface BookingChartResponse { monthly: MonthlyBookingStat[]; totalYear: number; year: number; }
...
const DONUT_FALLBACK = ['#f59e0b', '#3b82f6', '#10b981', '#ef4444'];
...
function LineChart({ data }: { data: MonthlyBookingStat[] }) {
  ...
  const W = 600, H = 200, PAD = { top: 20, right: 20, bottom: 36, left: 36 };
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[480px]" style={{ height: 220 }}>
        <defs>
        ...
        {gridLines.map((g, i) => (
          <g key={i}>
            ...
            <text x={PAD.left - 6} y={g.y + 4} textAnchor="end" fill="#9ca3af" fontSize="10">{g.label}</text>
          </g>
        ))}
        ...
        {/* X axis labels */}
        {pts.map((pt, i) => (
          <text key={i} x={pt.x} y={H - 8} textAnchor="middle" fill="#9ca3af" fontSize="10">
            {pt.label}
          </text>
        ))}
        ...
        {pts.map((pt, i) => (
          <g key={i} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)}
            ...
            <circle cx={pt.x} cy={pt.y} r={hover === i ? 5 : 4}
              fill={hover === i ? '#f59e0b' : '#fff'}
              stroke="#f59e0b" strokeWidth="2.5"
              className="transition-all duration-150" />
            {hover === i && pt.count > 0 && (
              <g>
                <rect x={pt.x - 22} y={pt.y - 32} width={44} height={22} rx="6"
                  fill="#1f2937" opacity="0.88" />
                <text x={pt.x} y={pt.y - 16} textAnchor="middle" fill="white" fontSize="11" fontWeight="700">
                  {pt.count} đơn
                </text>
              </g>
            )}
        ...
        {data.map((d, i) => {
          ...
          return (
            <li key={i} className="flex items-center gap-2.5">
              ...
              <span className="text-gray-400 text-xs w-12 text-right">({d.percentage}%)</span>
            </li>
          );
        })}
      </ul>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          { label: 'Đơn hoàn thành', value: data.completedBookings, color: 'text-emerald-600', bg: 'bg-emerald-50' },
          { label: 'Đã thanh toán',  value: data.paidBookingsCount,  color: 'text-blue-600',    bg: 'bg-blue-50' },
          { label: 'Chưa thanh toán', value: data.unpaidBookingsCount, color: 'text-amber-600', bg: 'bg-amber-50' },
        ].map(m => (
          <div key={m.label} className={`${m.bg} rounded-xl p-4`}>
            <p className="text-xs text-gray-500 mb-1">{m.label}</p>
            <p className={`text-xl font-extrabold ${m.color}`}>{m.value}</p>
            <p className="text-xs text-gray-400 mt-0.5">{(m.value * FEE_PER_BOOKING).toLocaleString('vi-VN')}đ</p>
          </div>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            <tr className="hover:bg-gray-50 transition">
              <td className="px-4 py-3 font-medium text-gray-700">Phí đã thanh toán</td>
              <td className="px-4 py-3 text-right text-gray-600">{data.paidBookingsCount}</td>
              <td className="px-4 py-3 text-right text-gray-400">{FEE_PER_BOOKING.toLocaleString('vi-VN')}đ</td>
              <td className="px-4 py-3 text-right font-bold text-emerald-600">{data.paidAmount.toLocaleString('vi-VN')}đ</td>
            </tr>
            <tr className="hover:bg-gray-50 transition">
              <td className="px-4 py-3 font-medium text-gray-700">Phí chưa thanh toán</td>
              <td className="px-4 py-3 text-right text-gray-600">{data.unpaidBookingsCount}</td>
              <td className="px-4 py-3 text-right text-gray-400">{FEE_PER_BOOKING.toLocaleString('vi-VN')}đ</td>
              <td className="px-4 py-3 text-right font-bold text-amber-600">{data.unpaidAmount.toLocaleString('vi-VN')}đ</td>
            </tr>
            <tr className="bg-gray-50/70">
              <td className="px-4 py-3 font-bold text-gray-800" colSpan={3}>Tổng phí dịch vụ tích lũy</td>
              <td className="px-4 py-3 text-right font-extrabold text-gray-900">{data.totalAmount.toLocaleString('vi-VN')}đ</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default function ManagerStats() {
  ...
  const monthlyQ = useQuery<BookingChartResponse>({
    ...
    queryFn: () => api.get(`/api/stats/manager/${restaurantId}/monthly-bookings?year=${year}`).then(r => r.data),
  ...
  if (!restaurantId) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-gray-400 gap-3">
        <svg className="w-12 h-12 opacity-30" fill="none" stroke="currentColor" strokeWidth={1.4} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        </svg>
        <p className="font-semibold text-gray-600">Tài khoản chưa liên kết nhà hàng</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Phân tích dữ liệu hoạt động · Nhà hàng #{restaurantId}</p>
      </div>
      ...
      >
        {monthlyQ.isLoading ? <Loading /> :
          monthlyQ.data && monthlyQ.data.monthly.length > 0 ? (
            <LineChart data={monthlyQ.data.monthly} />
          ) : (
            <p className="text-gray-400 text-sm">Chưa có dữ liệu cho năm {year}</p>
          )}
      </Card>

      {/* Status donut */}
      <Card title="🍩 Phân bổ trạng thái đặt bàn" subtitle="Tỉ lệ toàn bộ đơn trên hệ thống">
        {statusQ.isLoading ? <Loading /> :
          statusQ.data && statusQ.data.distribution.length > 0 ? (
            <StatusDonut data={statusQ.data.distribution} total={statusQ.data.totalBookings} />
          ) : (
