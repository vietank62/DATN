...
export function RefundProgress({ bookingId }: { bookingId: number }) {
  ...
  return <section className="space-y-3 rounded-xl bg-emerald-50 p-4"><h4 className="font-bold">{status}</h4><RefundTimeline refund={q.data} />{q.data.status === "pending" && <Link to={`/account/bookings/${bookingId}/refund`} className="inline-block rounded-lg bg-emerald-700 px-4 py-2 font-semibold text-white">{t("refund.addDetails")}</Link>}</section>;
}
