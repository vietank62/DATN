...
const ICONS = {
  ...
  users:        'M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m9-8a4 4 0 11-8 0 4 4 0 018 0zm6 8a2 2 0 100-4 2 2 0 000 4zM3 20a2 2 0 100-4 2 2 0 000 4z',
  restaurant:   'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2 9m13-9l2 9M9 22a1 1 0 110-2 1 1 0 010 2zm6 0a1 1 0 110-2 1 1 0 010 2z',
...
export default function DashboardSidebar({ navItems, brandLabel, variant }: DashboardSidebarProps) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center gap-3 px-4 py-5 border-b border-gray-100 ${collapsed ? 'justify-center' : ''}`}>
        <div className={`flex-shrink-0 w-9 h-9 rounded-xl bg-white flex items-center justify-center shadow-sm ring-2 ${LOGO_RING[variant]}`}>
          <svg className={`w-5 h-5 ${BRAND_COLOR[variant]}`} fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z"/>
          </svg>
        </div>
        {!collapsed && (
          <div>
            <p className={`text-sm font-bold ${BRAND_COLOR[variant]} leading-tight`}>TableNow</p>
      ...
      <nav className="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto">
        ...
        {navItems.map(item => (
          ...
          >
            {({ isActive }) => (
              <>
                <span className={isActive ? ACTIVE_ICON[variant] : 'text-gray-400'}>
      ...
      <div className="px-2 pb-4 space-y-1 border-t border-gray-100 pt-3">
        ...
        >
          <Icon path={collapsed ? ICONS.chevronRight : ICONS.chevronLeft} className="w-4 h-4 flex-shrink-0" />
          {!collapsed && <span>Thu gọn</span>}
        </button>
        <button
          onClick={handleLogout}
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-rose-500 hover:bg-rose-50 hover:text-rose-600 transition-all ${collapsed ? 'justify-center' : ''}`}
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className={`hidden lg:flex flex-col flex-shrink-0 h-screen bg-white border-r border-gray-200 sticky top-0
      ...
      >
        <Icon path={mobileOpen ? ICONS.close : ICONS.hamburger} className="w-5 h-5" />
      </button>
    </>
  );
}
