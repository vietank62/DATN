...
def create_booking(booking_data: BookingCreate, background_tasks: BackgroundTasks, session: SessionDep, current_user: Annotated[User, Depends(get_current_user)]):
    ...
    if booking.contactEmail:
        ...
        background_tasks.add_task(send_booking_email, booking.contactEmail, f"🔔 [TableNow] Yêu cầu đặt bàn của Quý khách tại {restaurant_name} đã được ghi nhận", email_content)
...
def get_bookings_by_restaurant_id(restaurant_id: int, session: SessionDep, current_user: Annotated[User, Depends(get_current_user)]):
    if current_user.role == "manager":
        ...
        if not restaurant or restaurant.managerID != current_user.userId:
...
def confirm_booking(id: int, background_tasks: BackgroundTasks, session: SessionDep, current_user: Annotated[User, Security(get_current_user, scopes=["manager"])], assignedSeats: int = None):
    ...
    if restaurant.availableSeats < seats_to_assign:
        raise HTTPException(
            status_code=400, 
            detail=f"Nhà hàng không đủ bàn trống. Chỉ còn {restaurant.availableSeats} chỗ khả dụng."
        )
    ...
    if booking.contactEmail:
        ...
        background_tasks.add_task(send_booking_email, booking.contactEmail, f"🎉 [TableNow] Xác nhận đặt bàn THÀNH CÔNG tại {restaurant.name} - Đơn #{booking.bookingId}", email_content)
...
def cancel_booking(id: int, background_tasks: BackgroundTasks, session: SessionDep, current_user: Annotated[User, Depends(get_current_user)]):
    ...
    if booking.status == "confirmed":
        ...
        # Trả lại ghế cho nhà hàng (nếu assignedSeats == 0 thì dùng requestSeats do dữ liệu cũ)
        seats_to_restore = booking.assignedSeats if booking.assignedSeats and booking.assignedSeats > 0 else booking.requestSeats
        restaurant.availableSeats += seats_to_restore
        if restaurant.availableSeats > restaurant.totalSeats:
            restaurant.availableSeats = restaurant.totalSeats
    ...
    if booking.contactEmail:
        email_content = (
            ...
            f"--------------------------------------------------\n"
            f"📌 THÔNG TIN ĐƠN ĐẠT BÀN ĐÃ HUỶ:\n"
            f"--------------------------------------------------\n"
            f"• Mã đặt bàn: #{booking.bookingId}\n"
            f"• Nhà hàng: {restaurant.name}\n"
            f"• Thời gian đã đặt: {booking.time} - Ngày {booking.date}\n"
            f"--------------------------------------------------\n\n"
            f"Nếu việc huỷ đặt bàn này là do sự thay đổi kế hoạch đột xuất, chúng tôi hy vọng sẽ có cơ hội được phục vụ Quý khách trong những dịp tụ họp tiếp theo. Quý khách có thể truy cập website TableNow bất kỳ lúc nào để khám phá và đặt trước hàng ngàn nhà hàng ẩm thực độc đáo khác với nhiều ưu đãi hấp dẫn.\n\n"
            f"Kính chúc Quý khách luôn có những kế hoạch vui tươi và tràn đầy hạnh phúc!\n\n"
            f"Trân trọng,\n"
            f"Đội ngũ TableNow"
        )
        background_tasks.add_task(send_booking_email, booking.contactEmail, f"❌ [TableNow] Thông báo huỷ đặt bàn #{booking.bookingId} thành công", email_content)
